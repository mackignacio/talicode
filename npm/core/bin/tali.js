...
//
// Implements #28. Launcher for the `tali` command shipped by @talicode/core.
//
  ...
  try {
    ...
    fail(
      `TaliCode: the platform package "${pkg}" is not installed.\n` +
        `It should install automatically as an optional dependency of @talicode/core.\n` +
        `Try reinstalling: npm install -g @talicode/core`
    );
  }
}
